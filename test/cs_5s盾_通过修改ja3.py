...
while True:
    try:
        ...
        print(url)
        logger.info(1)
        response = requests.get(url=url, headers=cf_random_headers(url), impersonate=random.choice(supported_browser_versions), proxies=proxy_jlsd(), verify=False, timeout=60)
        logger.info(2)
