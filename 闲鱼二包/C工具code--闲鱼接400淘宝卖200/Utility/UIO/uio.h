...
/* The file contains the the tools on 
- user-interface (like handling of a menu system), 
- input  
    - receive data from the keyboard
    - receive data from  files on the hard drive. 
- output
    - print messages for various purposes 
    - putting data to files on the  hard drive

Note:  The functions of reading and writing files are in this file. 
*/
...
/* Wait until the user hit enter. Print some hint message before user's input. The input queue is cleared after calling this function. .  */

/**
 * @brief Pause the program until the user type the Enter key, or a line of text ended by Enter. 
 * @note It should be called only when the input queue of STDIN is empty. Otherwise, it will not pause the program, just clear the input queue. 
 ...
 * @param fp   A file stream, which should be already be opened and ready for output.
  * @return Write the characters of cs one by one into the file fp, until all characters are written, or something is wrong.  Return the number of characters written to the file. 
